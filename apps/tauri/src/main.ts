      ...
      <article class="panel quick-access-manage-panel">
        ...
        ${
          currentBindings.length
            ? `<div class="quick-access-manage-list">${currentBindings.map((binding) => `<article class="quick-access-manage-row"><div class="quick-access-manage-main"><span class="quick-access-manage-icon">${icon(bindingIcon(binding.action))}</span><div><h3>${escapeHtml(binding.label)}</h3><p>${escapeHtml(describeBindingAction(binding.action))}</p></div></div><label class="toggle-line quick-access-toggle"><input class="favorite-toggle" data-binding-id="${escapeHtml(binding.id)}" type="checkbox" ${binding.favorite ? "checked" : ""} /><span>${binding.favorite ? "Shown" : "Hidden"}</span></label></article>`).join("")}</div>`
